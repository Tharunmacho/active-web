...
const BusinessDashboard = () => {
    ...
    const loadData = async () => {
        try {
            ...
            // Load active company only with cache busting
            const activeCompanyResponse = await fetch(`http://localhost:4000/api/companies/active?t=${Date.now()}`, {
                headers: {
            ...
            console.log("📦 Active company response:", activeCompanyResult);
            
            if (activeCompanyResult.success && activeCompanyResult.data) {
                console.log("✅ Setting active company:", activeCompanyResult.data.businessName);
                setActiveCompany(activeCompanyResult.data);
            } else {
                console.log("⚠️ No active company found");

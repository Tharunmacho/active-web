      ...
      try {
        ...
        if (personalFormResponse.ok) {
          ...
          if (personalResult.data) {
            personalData = personalResult.data;
            console.log("Loading personal form from backend:", personalData);
            
            setHasExistingProfile(true);
            setIsLocked(personalData.isLocked || false);

            // Load districts if state exists
            if (personalData.state) {
              console.log("Setting districts for state:", personalData.state);
              setDistricts(INDIA_DISTRICTS[personalData.state] ?? []);
              
              // Load blocks if district exists
              if (personalData.district) {
                const blocksUrl = `http://localhost:4000/api/locations/states/${encodeURIComponent(personalData.state)}/districts/${encodeURIComponent(personalData.district)}/blocks`;
        ...
        // Load financial form data
        console.log("Fetching financial form data...");
        const financialFormResponse = await fetch("http://localhost:4000/api/financial-form", {
          headers: {
            "Authorization": `Bearer ${token}`,
            "Content-Type": "application/json"
          }
        ...
        console.log("Financial form response status:", financialFormResponse.status);
        
        let financialData: any = {};
        if (financialFormResponse.ok) {
          const financialResult = await financialFormResponse.json();
          console.log("Financial form result:", financialResult);
          
          if (financialResult.data) {
            financialData = financialResult.data;
            console.log("Loading financial form from backend:", financialData);
          }
        }

        // Load declaration form data
        console.log("Fetching declaration form data...");
        const declarationFormResponse = await fetch("http://localhost:4000/api/declaration-form", {
          headers: {
            "Authorization": `Bearer ${token}`,
            "Content-Type": "application/json"
          }
        ...
        console.log("Declaration form response status:", declarationFormResponse.status);
        
        let declarationData: any = {};
        if (declarationFormResponse.ok) {
          const declarationResult = await declarationFormResponse.json();
          console.log("Declaration form result:", declarationResult);
          
          if (declarationResult.data) {
            declarationData = declarationResult.data;
            console.log("Loading declaration form from backend:", declarationData);
          }
        }

        // Combine all form data and update the form
        const formValues = {
          // Personal form data
          ...
          socialCategory: personalData.socialCategory || "",
          password: "",
          confirmPassword: "",
          currentPassword: "",
          // Business form data
          ...
          // Financial form data
          pan: financialData.pan || "",
          gst: financialData.gst || "",
          udyam: financialData.udyam || "",
          filedITR: financialData.filedITR || "",
          itrYears: financialData.itrYears || "",
          turnoverRange: financialData.turnoverRange || "",
          turnover1: financialData.turnover1 || "",
          turnover2: financialData.turnover2 || "",
          turnover3: financialData.turnover3 || "",
          govtSchemes: financialData.govtSchemes || "",
          scheme1: financialData.scheme1 || "",
          scheme2: financialData.scheme2 || "",
          scheme3: financialData.scheme3 || "",
          // Declaration form data
          sisterConcerns: declarationData.sisterConcerns || "",
          companyNames: declarationData.companyNames || "",
          declarationAccepted: declarationData.declarationAccepted || false
        };
        
        console.log("Setting all form values:", formValues);
        reset(formValues);
      } catch (error) {
